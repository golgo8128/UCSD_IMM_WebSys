...
def plan_info(request, plan_objid):
    ...
    plan_links_from_this = WorkChunk_Plan_Link.objects.filter(plan_src__id = plan_objid)
    plan_links_to_this   = WorkChunk_Plan_Link.objects.filter(plan_tgt__id = plan_objid)
    ...
    templt = loader.get_template("appWorkChunkPlanner/plan_info4.html")
    return HttpResponse(templt.render(contxt))  
  
  
def plan_accept(request, plan_objid):
    
    contxt = RequestContext(request, { })
    templt = loader.get_template("appWorkChunkPlanner/plan_accept1.html")
