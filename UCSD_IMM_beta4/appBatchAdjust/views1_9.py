...
from Usefuls.Str2Num_except1 import str2num

from pNephrology.MetabDatProc1.Rplot_batch_PDFout1_3 import invoke_Rplot_batch_PDFout
from pNephrology.MetabDatProc1.Metab_Batch_Adjust_SingleFile1_14 \
    ...
    fupobjs = \
        FileUploaded.objects.filter(idatatype =
            DataType.objects.get(datatype =
                "Metab levels in each batch")).order_by("-itimestamp_touch")[:LAST_N]

    contxt = RequestContext(request,
                            { "fupobjs" : fupobjs })
    templt = loader.get_template("appBatchAdjust/proc_singlefile1_9.html")
...
def invoke(request):
    ...
    django_fileobj = FileUploaded.objects.get(id = request.POST["selfilobj"])
    django_fileobj.itimestamp_touch = timezone.now()
    django_fileobj.save()
    input_path_raw = django_fileobj.ifile.name
    ...
    oxls_bfilnam = uniqstamp.stamp_on("%s_badj1_7.xlsx")
    otsv_bfilnam = uniqstamp.stamp_on("%s_badj1_7.tsv")
    itsv_bfilnam = uniqstamp.stamp_on("%s_input1_7.tsv")
    opdf_bfilnam = uniqstamp.stamp_on("appBA1_7_%s.pdf")
    ...
    # metabdetectlowlimit  = float(request.POST[ "metabdetectlowlimit" ])
    
    metabdetectllim_ctrl_bfr = str2num(request.POST.get("metabdetectllim_ctrl_bfr", None))
    metabasnsmallqt_ctrl_bfr = str2num(request.POST.get("metabasnsmallqt_ctrl_bfr", None))
    metabdetectllim_ctrl_aft = str2num(request.POST.get("metabdetectllim_ctrl_aft", None))
    metabasnsmallqt_ctrl_aft = str2num(request.POST.get("metabasnsmallqt_ctrl_aft", None))
    
    metabdetectllim_act_bfr = str2num(request.POST.get("metabdetectllim_act_bfr", None))
    metabasnsmallqt_act_bfr = str2num(request.POST.get("metabasnsmallqt_act_bfr", None))
    metabdetectllim_act_aft = str2num(request.POST.get("metabdetectllim_act_aft", None))
    metabasnsmallqt_act_aft = str2num(request.POST.get("metabasnsmallqt_act_aft", None))
    
    forceNaN_lowlim_h = {
        "before" : { "ctrl" : metabdetectllim_ctrl_bfr,
                     "act"  : metabdetectllim_act_bfr
                   },
        "after"  : { "ctrl" : metabdetectllim_ctrl_aft,
                     "act"  : metabdetectllim_act_aft
                   }
    }

    assign_smallqt_h = {
        "before" : { "ctrl" : metabasnsmallqt_ctrl_bfr,
                     "act"  : metabasnsmallqt_act_bfr
                   },
        "after"  : { "ctrl" : metabasnsmallqt_ctrl_aft,
                     "act"  : metabasnsmallqt_act_aft
                   }
    }
    ...
    ctrlmedian_outfactor = float(request.POST[ "ctrlmedian_outfactor" ])
    ctrl_cv_thres        = float(request.POST[ "ctrl_cv_thres" ])
    ...
    contxt = RequestContext(request,
                            { "PDFfile"  : BADJ_PDF_DIRURL.get_user_urlpath(opdf_bfilnam),
                              ...
                              "mn_func_sel"          : mn_func_sel,
                              "metabdetectllim_ctrl_bfr" : str(metabdetectllim_ctrl_bfr),
                              "metabdetectllim_act_bfr"  : str(metabdetectllim_act_bfr),
                              "metabdetectllim_ctrl_aft" : str(metabdetectllim_ctrl_aft),
                              "metabdetectllim_act_aft"  : str(metabdetectllim_act_aft),
                              "metabasnsmallqt_ctrl_bfr" : str(metabasnsmallqt_ctrl_bfr),
                              "metabasnsmallqt_act_bfr"  : str(metabasnsmallqt_act_bfr),
                              "metabasnsmallqt_ctrl_aft" : str(metabasnsmallqt_ctrl_aft),
                              "metabasnsmallqt_act_aft"  : str(metabasnsmallqt_act_aft),
                              "ctrl_cv_thres"        : str(ctrl_cv_thres),
                              "ctrlmedian_outfactor" : str(ctrlmedian_outfactor),                              
                              })
    templt = loader.get_template("appBatchAdjust/badj_dl_links1_4.html")
    ...
    uniqstamp.\
        write_json_stamped_file(
            { "chk_base_filename"    : outbasefilnam,
              ...
              # "mn_func_sel"          : mn_func_sel,
              # "smallvalforzerosnans" : str(smallvalforzerosnans),
              # "ctrl_cv_thres"        : str(ctrl_cv_thres),
              # "ctrlmedian_outfactor" : str(ctrlmedian_outfactor),
              })
...
def initiate(istamp,
    ...
    try:
        ...
        mbas = Metab_Batch_Adjust_SingleFile(
            ...
            calcmnfunc             = mn_func,
            forceNaN_lowlim_h      = forceNaN_lowlim_h,
            assign_smallqt_h       = assign_smallqt_h,
            colname_batch_no = COLNAM_BATCHNUM,
            colname_sampleid = COLNAM_SAMPLEID)

        mbas.adjust_write(oxls_fpath, otsv_fpath, itsv_fpath,
                          erase_ctrl_odist_rate = ctrlmedian_outfactor,
                          erase_ctrl_cv_thres = ctrl_cv_thres)
