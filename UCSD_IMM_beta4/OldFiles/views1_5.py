...
from FileDirPath.File_Path_Conv1_2 import file_path_conv_guess
...
BADJ_TSV_DIRURL_SUB = "/".join(("appBatchAdjust", "TSV")) # Not appropriate for Windows?
BADJ_PDF_DIRURL_SUB = "/".join(("appBatchAdjust", "PDF"))
BADJ_XLS_DIRURL_SUB = "/".join(("appBatchAdjust", "XLS"))

# Notice:
# MEDIA_ROOT should end with "/"
# BADJ_...DIRURL_SUB should NOT start with "/"
BADJ_TSV_DIR    = os.path.join(settings.MEDIA_ROOT,
                               BADJ_TSV_DIRURL_SUB )
BADJ_PDF_DIR    = os.path.join(settings.MEDIA_ROOT,
                               BADJ_PDF_DIRURL_SUB)
BADJ_XLS_DIR    = os.path.join(settings.MEDIA_ROOT,
                               BADJ_XLS_DIRURL_SUB)
...
def invoke(request):
    ...
    oxls_fpath   = os.path.join(BADJ_XLS_DIR, oxls_bfilnam)
    otsv_fpath   = os.path.join(BADJ_TSV_DIR, otsv_bfilnam)
    ...
    pdfout_merged_fpath = \
        os.path.join(BADJ_PDF_DIR, opdf_bfilnam)
    ...
    contxt = RequestContext(request,
                            { "PDFfile" : "/".join((BADJ_PDF_DIRURL_SUB,
                                                    opdf_bfilnam )) })
...
def initiate(istamp,
    ...
    try:
        ...
        mbas.adjust_write(oxls_fpath, otsv_fpath)
    ...
    except ZeroDivisionError as inst: # Exception as inst: 
