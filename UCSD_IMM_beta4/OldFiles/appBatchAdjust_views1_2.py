...
def entry(request):
    ...
    contxt = RequestContext(request,
                            { "fupobjs" : fupobjs })
    templt = loader.get_template("appBatchAdjust/proc_singlefile1_4.html")
...
def invoke(request):
    ...
    cetime = hex(int(time.time() * 100000))[2:]
    outbasefilnam = "tmpout11_%s_done.xlsx" % cetime
    errbasefilnam = "tmpout11_%s_errr.txt"  % cetime
